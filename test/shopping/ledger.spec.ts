...
describe('budget rules with mocked persistence', () => {
  it('admits reservations only within the monthly cap', async () => {
    ...
    const results = [];
    for (let i = 0; i < 110; i++)
      results.push(await stub.reserve(correlation(), PRICE.version));
  ...
  it('keeps missing usage reserved through month rollover; late usage changes the original bucket', async () => {
    ...
    expect((await ledger(storage).reserve(call, PRICE.version)).status).toBe(
      'duplicate',
    );
    expect((await storage.getReservation(reservation.id))?.charged).toBe(
      RESERVATION,
    );
    ...
    expect(await storage.getReservation(reservation.id)).toMatchObject({
      month: '2000-01',
      status: 'settled',
      charged: 650,
    });
    expect(await storage.totalCharged('2000-02')).toBe(RESERVATION);
  });
