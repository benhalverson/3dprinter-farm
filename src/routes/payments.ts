...
import { productsTable, cart, users } from '../db/schema';
...
const paymentsRouter = factory
	...
	.get(
		'/success',
		describeRoute({
			description: 'Stripe payment success callback page. Users are redirected here after successful payment. Can include session_id query parameter for verification.',
	...
	.get(
		'/cancel',
		describeRoute({
			description: 'Stripe payment cancellation callback page. Users are redirected here when they cancel payment or payment fails.',
			tags: ['Payments', 'Stripe'],
			responses: {
				200: {
	...
	.post(
		'/paypal',
		describeRoute({
			description: 'Create PayPal payment order. Legacy endpoint that creates a PayPal order with quantity-based pricing. Uses sandbox PayPal API.',
			...
			parameters: [
				{
					...
					description: 'Quantity multiplier for pricing (qty * 10 = total price)'
				}
			],
			responses: {
				200: {
	...
	.post(
		'/webhook/stripe',
		describeRoute({
			description: 'Handle Stripe webhook events for payment confirmation. This endpoint processes checkout.session.completed events, creates orders with Slant3D API, and clears the cart. Must be configured in Stripe Dashboard with proper webhook secret.',
			tags: ['Payments', 'Stripe', 'Webhooks'],
			responses: {
				200: {
					description: 'Webhook processed successfully - order created and cart cleared',
					content: {
						'application/json': {
							example: {
								success: true,
		...
		try {
			...
			if (event.type === 'checkout.session.completed') {
				...
				try {
					...
					if (!response.ok) {
						console.error('Slant3D order creation failed:', response.status, await response.text());
					...
					return c.json({ success: true, orderId: orderResponse.orderId || 'created' });
				} catch (error) {
