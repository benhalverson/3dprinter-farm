...
const webhookRoutes = factory.createApp().post('/webhook/stripe', async c => {
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      console.log(
        `Checkout session completed: ${session.id}, cartId: ${cartId}`,
      );
    ...
    if (event.type === 'payment_intent.succeeded') {
      ...
      console.log(
        `Payment intent succeeded: ${paymentIntent.id}, cartId: ${cartId}`,
      );
