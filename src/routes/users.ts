...
import { createSecretKey, encrypt as ckEncrypt, decrypt as ckDecrypt, isInWebApiEncryptionFormat } from 'cipher-kit/web-api';
...
const userRouter = factory
	...
	.get(
		...
		async (c) => {
			...
			const secretKeyCache: Map<string, WebApiKey> = (globalThis as any).__profileSecretKeyCache || new Map();
			...
			const getSecretKey = async (passphrase: string): Promise<WebApiKey> => {
				...
				if (!res.success) throw new Error(`cipher-kit key derivation failed: ${res.error.message} - ${res.error.description}`);
				secretKeyCache.set(passphrase, res.secretKey);
				return res.secretKey;
			};

			const decryptValue = async (value: string | null, passphrase: string, secretKey: WebApiKey): Promise<string | null> => {
				...
				try {
					...
					if (isInWebApiEncryptionFormat(value)) {
						...
						if (!dec.success) throw new Error(`cipher-kit decrypt error: ${dec.error.message}`);
					...
					console.warn('Profile decrypt fallback triggered for value, returning empty string. Reason:', e);
					return '';
				}
			};
			...
			try {
				...
				const decryptedProfile = {
					...
					firstName: await decryptValue(userData.firstName, passphrase, secretKey),
					lastName: await decryptValue(userData.lastName, passphrase, secretKey),
					address: await decryptValue(userData.shippingAddress, passphrase, secretKey),
				...
				console.log('Decrypted profile for user ID:', user.id, decryptedProfile);
	...
	.post(
		...
		authMiddleware,
		describeRoute({
			description: 'Update the profile of the authenticated user (no id param required)',
			tags: ['User'],
			responses: {
		...
		async (c) => {
			...
			const encryptValue = async (value: string | null, secretKey: WebApiKey): Promise<string | null> => {
				if (value == null || value === '') return value; // store empty/nullable as-is
				const enc = await ckEncrypt(value, secretKey);
				if (!enc.success) throw new Error(`cipher-kit encrypt error: ${enc.error.message} - ${enc.error.description}`);
			...
			try {
				...
				const [userData] = await c.var.db
					...
					.set({
						...
						shippingAddress: await encryptValue(shippingAddress, secretKey) ?? '',
	...
	.post(
		...
		authMiddleware,
		describeRoute({
			description: 'Update the profile of a user by ID (must match authenticated user)',
			tags: ['User'],
			responses: {
				200: { description: 'User profile updated successfully' },
				400: { description: 'Validation failed' },
